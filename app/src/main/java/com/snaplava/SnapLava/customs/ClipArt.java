...
public class ClipArt extends RelativeLayout {
	...
	int pivy;
	Activity activity;
	int pos;float rotation_angle =0;
	Bitmap shadowBitmap;
	float startDegree;
	String[] v;

	public ClipArt(Context paramContext, final Activity activeity) {
		super(paramContext);
		cntx = paramContext;
		activity = activeity;
		layGroup = this;
		...
		//clip_view = (ImageView) findViewById(R.id.clipart);
...
if(layoutParams == null) {
	layoutParams = new LayoutParams(250, 250);
		...
		image.setImageResource(R.drawable.ic_message);
			...
			public boolean onTouch(View paramAnonymousView, MotionEvent event) {
				if (!ClipArt.this.freeze) {
					...
					case MotionEvent.ACTION_MOVE:
						...
						rotation_angle=(ClipArt.this.startDegree + i) % 360.0F;
						ClipArt.this.layGroup.setRotation(rotation_angle);
						break;
					}
		...
		this.btndel.setOnClickListener(new OnClickListener() {
			public void onClick(View paramAnonymousView) {
				if (!ClipArt.this.freeze) {
					...
					((CollageActivity)activeity).setRefered(true);
					layBg.performClick();
	...
	public void setbaseY(LayoutParams layouTParams){
...
public void setAngle(float angle ){
	...
	ClipArt.this.layGroup.setRotation(angle);
}
